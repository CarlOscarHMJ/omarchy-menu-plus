...
Item {
  ...
  property int cardWidth: Math.min(root.dmenuActive ? Style.space(root.dmenuWidth) : ((root.activeMenu === "trigger.capture.screenrecord" || root.activeMenu === "style.font" || root.isFileSearchQuery(root.filterText.trim())) ? Style.space(520) : Style.space(300)), panel.width - Style.gapsOut * 2)
  ...
  // "." prefix: whole-filesystem file search (walker's old `.` mode).
  ...
  // "=" prefix: inline calculator (walker's old `=` mode). Evaluated
  // synchronously — arithmetic only, no filesystem I/O, so no Process needed.
  function isCalcQuery(text) {
    return text.length > 1 && text.charAt(0) === "="
  }

  // Math functions/constants allowed in calc expressions, keyed by their
  // lowercase spelling (so "SQRT"/"Sqrt"/"sqrt" all work) mapping to the
  // real Math.* member. Anything else stays rejected before ever reaching
  // Function() — keeps this an arithmetic evaluator, not a general eval sink.
  ...
  function evaluateCalc(expr) {
    ...
    for (var i = 0; i < identifiers.length; i++) {
      if (!(identifiers[i].toLowerCase() in root.calcFunctions)) return null
    }

    var sanitized = trimmed.replace(/\^/g, "**")
    sanitized = sanitized.replace(/[a-zA-Z_][a-zA-Z0-9_]*/g, function(match) {
      return "Math." + root.calcFunctions[match.toLowerCase()]
    })
  ...
  function startFileSearch(term) {
    if (fileSearchProc.running) {
      root.fileSearchPendingQuery = term
      return
    }
    ...
    // implicit wrapping, so "foo*bar" would only match a path that literally
    // starts with "foo" and ends with "bar". Wrap it ourselves so both plain
    // substring queries and explicit wildcard queries behave the same way.
    var pattern = term
    if (pattern.charAt(0) !== "*") pattern = "*" + pattern
    if (pattern.charAt(pattern.length - 1) !== "*") pattern = pattern + "*"
    fileSearchProc.collected = ""
    fileSearchProc.command = ["bash", "-lc", "locate -i --limit 60 -- " + Util.shellQuote(pattern)]
  ...
  function rebuildDisplay() {
    ...
    var trimmedForMode = root.filterText.trim()

    if (!root.dmenuActive && root.isFileSearchQuery(trimmedForMode)) {
    ...
    if (!root.dmenuActive && root.isCalcQuery(trimmedForMode)) {
      ...
      if (calcResult !== null) {
        ...
        displayModel.append({
          ...
          label: resultText,
          target: "",
          detail: trimmedForMode,
          path: "",
          childCount: 0,
          action: "printf '%s' " + Util.shellQuote(resultText) + " | wl-copy",
          provider: "",
          score: 0,
          section: ""
        })
